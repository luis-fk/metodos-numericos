...
double edo(double factor, int body1, int body2, int body3, bodyInfo *body, int coordinate);
double calculateDistance(bodyInfo *body, int body1, int body2, double factor);
...
int main() {
    ...
    /* Stores the initial data for position, velocity and mass of each body */
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 3; j++) {
            data >> number;
...
void RungeKuta44(bodyInfo *body, double timeInterval) {
    ...
    for (int i = 0; i < 3; i++) {
        /* Iteration for each on of the coordinates (x, y, z) */
        for (int k = 0; k < 3; k++) {
            body[i].position[k] = body[i].updatedPosition[k];
        }
    }
...
void phiFunction(bodyInfo *body, double timeInterval, int body1, int body2, 
    ...
    k1 = edo(0, body1, body2, body3, body, coordinate);
    k2 = edo(k1*(timeInterval/2), body1, body2, body3, body, coordinate);
    k3 = edo(k2*(timeInterval/2), body1, body2, body3, body, coordinate);
    k4 = edo(k3*timeInterval, body1, body2, body3, body, coordinate);
    phi = (k1 + 2*k2 + 2*k3 + k4)/6;

    body[body1].updatedPosition[coordinate] = body[body1].position[coordinate] + timeInterval*body[body1].velocity[coordinate];

...
def main():
    ...
    for j in range(m):
        ...
        for i in range(k-2):
            plt.plot(imagens_x[j][i], imagens_y[j][i], label = f'n = {nArr[i]}', color='black')  # Plotando apenas a primeira coluna de y_k
            plt.xlabel('Tempo')
            plt.ylabel(f'y$_{j}$(t)')
        plt.title(f'Convergência do Método de Runge-Kutta de 4ª ordem para a variável y$_{j+1} $')
        plt.savefig(f'imagens/RK44SM/convergenciaRK44SM{j+1}.png')
        plt.clf()
        ...
        plt.plot(imagens_x[j][-1], imagens_y[j][-1], color='black', linestyle=(0,(1,2,3,2)),
        ...
        plt.savefig(f'imagens/RK44SM/comparacaoRK44SM{j+1}.png')
        plt.clf()
    ...
    with open('inputs-outputs/tablesRK44SM.txt', 'w') as file:

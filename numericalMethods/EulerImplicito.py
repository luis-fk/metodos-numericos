...
def Eimplicito(t0, tfin, xin, yin, n, F):
    ...
    while i<=n:
        tk=t0 + dt
        c=1/((1-(dt/tk)) + 4*((dt)**2)*((tk)**2)) #constante auxiliar
        xk= ((1-(dt/tk))*xin +dt*yin)*c # foi aplicado o método de euler implicito nas duas equações à serem resolvidas e
        yk=((-4*(tk**2)*dt*xin) +yin)*c # foi isolado o x(tk+1) e y(tk+1).
        xsol.append(xk)
        ysol.append(yk)
        tsol.append(tk)
        t0=tk
...
def ErroImp(t0, tfin, xin, yin, m, F):
    ...
    while i<=14:
        ...
        qtdm.append(i)
        tabh.append(h)
        listaxE=Eimplicito(t0, tfin, xin, yin, n, F)[1]
        listayE=Eimplicito(t0, tfin, xin, yin, n, F)[2]
        xaE=listaxE[-1]
        yaE=listayE[-1]
        xe=np.sin((tfin)**2)
        ye=2*(tfin)*np.cos((tfin)**2)
...
while opcao!=8:
    ...
    if opcao==1:
        ...
        testeCaso2I=ErroICaso2(np.sqrt(np.pi), (np.sqrt(np.pi))+1, 0, -2*(np.sqrt(np.pi)), 2, F)
        print(testeCaso2I)
    elif opcao==8:
        print('Finalizando...')
    else:
